...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        self.mlp_model = MLP(input_dim=2048, hidden_dim1=1024, hidden_dim2=512, output_dim=512, dropout_prob=0.3)

        self.model = CustomModel(num_regions=49, num_features=512, num_gaussian_kernels=5, final_dim=512)

        self.model_bilinear = WeightedBilinearModel(input_dim=512, weight_dim=49)
    ...
    def forward(
        ...
        if src_img_features is not None:
            ...
            feature_pair_tuple_tensor_1 = [x[0] for x in src_img_features]
            feature_pair_tuple_tensor_2 = [y[1] for y in src_img_features]

            feature_pair_tuple_tensor_1 = self.mlp_model(torch.cat(feature_pair_tuple_tensor_1, dim=0))
            feature_pair_tuple_tensor_2 = self.mlp_model(torch.cat(feature_pair_tuple_tensor_2, dim=0))

            output = self.model(feature_pair_tuple_tensor_1, feature_pair_tuple_tensor_2)

            representation_bilinear = self.model_bilinear((feature_pair_tuple_tensor_1, feature_pair_tuple_tensor_2), output)


            ########################For Fashion-MMT########################################


            # pattern = re.compile(r'/Fashion-MMT/(\d+)/')
            # final_output = []
            #
            ...
            # for item in src_img_features_location:
            #     for element in item:

            #         if isinstance(element, np.ndarray):

            #             prev_path = item[item.index(element) - 1]
            #             match = pattern.search(prev_path)
            #             if match:
            ...
            # for number, half_count in counts.items():

            #     each_folder_representation = representation_bilinear[current_idx:current_idx + half_count]
            #

            #     each_folder_final_output = torch.mean(torch.cat(each_folder_representation, dim=1), dim=1,keepdim=True).half()
            #     final_output.append(each_folder_representation)
            #

            #     current_idx += half_count


            ######Fashion-MMT#######
            # src_img_features = each_folder_final_output
            ########################


            ##########other dataset#####
            src_img_features = representation_bilinear
            ############################
        ...
        for idx, layer in enumerate(self.layers):
            ...
            if not self.training or (dropout_probability > self.encoder_layerdrop):
                if src_img_features is not None:
                    x, hallucination = layer(x, x_hallucination, encoder_padding_mask_text, encoder_padding_mask_hallucination, batch_len, idx, src_img_features=src_img_features)

                else:
                    x, hallucination = layer(x, x_hallucination, encoder_padding_mask_text, encoder_padding_mask_hallucination, batch_len, idx)
...
class GaussianKernel(nn.Module):
    def __init__(self, num_regions, num_features):
        ...
        self.sigma = nn.Parameter(torch.ones([])).to(device)
        self.mean = nn.Parameter(torch.zeros([])).to(device)
        self.num_regions = num_regions
        self.num_features = num_features

    def forward(self, x1, x2):
        distance = x1.unsqueeze(1).to(device) - x2.unsqueeze(2).to(device) - self.mean.half()

        distance_square = torch.sum(distance * distance, -1)

        similarity = torch.exp(-distance_square.float() / (2 * self.sigma * self.sigma)).half()
...
class CustomModel(nn.Module):
    def __init__(self, num_regions, num_features, num_gaussian_kernels, final_dim):
        ...
        self.sigma_parameters = nn.Parameter(torch.randn(num_gaussian_kernels)).to(device)
        self.weights = nn.Parameter(torch.randn(num_gaussian_kernels)).to(device)
        self.softmax = nn.Softmax(dim=0).to(device)
        self.bn = nn.BatchNorm1d(num_gaussian_kernels).to(device)
    ...
    def forward(self, x1, x2):
        ...
        for kernel, weight in zip(self.gaussian_kernels, self.weights):
            similarity_matrix = kernel(x1, x2).to(device)
            representations.append(similarity_matrix.unsqueeze(0))

        final_representation = torch.cat(representations, dim=0).to(device)
        ...
        normalized_weights_expanded = normalized_weights.unsqueeze(0).unsqueeze(-1).unsqueeze(-1).half().to(device)

        final_representation_weighted = (final_representation.transpose(0, 1) * normalized_weights_expanded).to(device)

        final_output = torch.sum(final_representation_weighted, dim=1).half().to(device)
...
class BilinearInteraction(nn.Module):
    ...
    def forward(self, x1, x2, similarity_matrix):

        x1_transformed = x1.squeeze(1).transpose(1, 2).to(device)

        interaction_part1 = torch.bmm(x1_transformed.to(device), similarity_matrix).to(device)

        interaction_part1 = interaction_part1.transpose(1, 2).unsqueeze(1).to(device)

        interaction = interaction_part1.to(device) * x2.to(device).to(device)
...
class WeightedBilinearModel(nn.Module):
    ...
    def forward(self, tuples, similarity_matrix):

        interactions = self.interaction(tuples[0], tuples[1], similarity_matrix).to(device)
        return interactions
