...
def collate_graph_tokens(values):
    """Convert a list of 1d tensors into a padded 2d tensor."""
    size =  max(v.size(1) for v in values)
    res = torch.zeros([len(values), size, 2048])

    def copy_tensor(src, dst):
        assert dst.size(0) == src.size(1)
        dst.copy_(src[0])

    for i, v in enumerate(values):
        tmp_src_len = len(res[i])
        v = torch.tensor(v)
        if v.size(1) > tmp_src_len:
            try:
                copy_tensor(v[1:len(res[i])+1, 1:len(res[i])+1], res[i])
            except:
                123
        elif v.size(1) == tmp_src_len:
            try:
                copy_tensor(v, res[i])
            except:
                123
            ...
            # print('else')
            try:
                # copy_tensor(v, res[i][1:(v.size(1) + 1), 1:(v.size(1) + 1)])
                copy_tensor(v,res[i][0: v.size(1), 0:])
                # copy_tensor(v, res[i][1:(tmp_src_len-v.size(1)), 0:])
            except:
                123
    return res
...
def collate_tokens1(values,  eos_idx=None, left_pad=False, move_eos_to_beginning=False):
    """Convert a list of 1d tensors into a padded 2d tensor."""
